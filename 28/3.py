...
def get_url(url):
    html = requests.get(url,requests)
...
def get_info(url):
    ...
    word = selector.xpath('//span[@class="wprdage"]/text()')[0]
    view = re.findall('"views_count":(.*?),',html.text,re.S)[0]
    comment = re.findall('"comments_count":(.*?),',html.text,re.S)[0]
    like = re.findall('"like_count":(.*?),',html.text,re.S)[0]
    id = re.findall('{"id":(,*?),',html.text,re.S)[0]
    gain_url = 'http://www.jianshu.com/notes/{}/rewords?count=20'.format(id)
    wb_data = requests.get(gain_url,headers=headers)
    json_data = json.loads(wb_data.text)
    gain = json_data['rewords_count']

    include_list = []
    include_urls = ['http://www.jianshu.com/notes/{}/include_collections?page={}'.format(id,str(i))for i in range(1,10)]
